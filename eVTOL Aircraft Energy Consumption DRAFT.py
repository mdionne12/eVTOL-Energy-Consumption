...
phi = 0 #Rotor tip-path-plane roll angle [degrees]


def powerCruise_noWind (F_d_value, alpha_value, **kwargs):
    ...
    T = m*g/(numpy.cos(phi*numpy.pi/180)*numpy.cos(Theta*numpy.pi/180)) #Total eVTOL thrust [N]
    T_rotor = T/4
    ...
    #Induced Velocity
    coefficients = [1, 2*V_cruise*numpy.sin(alpha_value*numpy.pi/180), (V_cruise**2), 0, -Vh**4] #Defining the coefficients of the polynomial
    ...
    y = (Vh**2)/(numpy.sqrt((V_cruise*numpy.cos(alpha_value*numpy.pi/180))**2+(V_cruise*numpy.sin(alpha_value*numpy.pi/180)+Vi)**2))
    ...
    #Parasite Power
    P_parasite = T*V_cruise*numpy.sin(alpha_value*numpy.pi/180) #[W]
